...
<#macro facetpagination facet url numNumbered=4 showbox=true>
<#-- the current page number, first page = 1 -->
  <#assign  page = action.getFacetPage(facet) />
  <#assign currPage = (page.offset/page.limit)?round + 1 />
<#-- the first numbered page to show. If only 1 page exists its a special case caught below -->
  <#assign firstNum = common.max(2, currPage - numNumbered +1) />
...
<ul class="numbered-pagination">
  ...
  <#if currPage gt 1>
    ...
    <#list firstNum .. currPage as p>
      <@pageLinkFacet title=p url=getFacetPageUrl(url, page.limit*(p-1),facet,showbox) current=(currPage=p) />
    </#list>
  </#if>
  <@pageLinkFacet title=">" url=getFacetPageUrl(url, page.limit*(currPage),facet,showbox) current=false />
</ul>
</#macro>
...
<#function getFacetPageUrl url offset facet showbox=true>
  <#assign fullUrl = "" >
  <#if url?contains("?")>
    <#assign fullUrl = url + "&" + facet + ".offset=" + offset?c>
  <#else>
    <#assign fullUrl = url + "?" + facet + ".offset=" + offset?c>
  </#if>
  <#if showbox>
    <#assign fullUrl = fullUrl + "&facetbox=" + facet >
  </#if>
