...
<tr>
    ...
    <td class="wordwrap"><#if respLink?has_content><a href="${action.cfg.apiBaseUrl}${respLink}" target="_blank">${resp}</a><#else>${resp}</#if></td>
    <td><#nested/></td>
...
<#if addSearchParams>
  <#assign params2={
  "hl": "Set hl=true to highlight terms matching the query when in fulltext search fields. The highlight will be an emphasis tag of class 'gbifH1' e.g. <a href='${action.cfg.apiBaseUrl}/dataset/search?q=plant&hl=true' target='_blank'>/search?q=plant&hl=true</a>. Fulltext search fields include: title, keyword, country, publishing country, owning organization title, hosting organization title, and description. One additional full text field is searched which includes information from metadata documents, but the text of this field is not returned in the response.",
  ...
  "facet_mincount": "Used in combination with the facet parameter. Set facet_mincount={#} to exclude facets with a count less than {#}, e.g. <a href='${action.cfg.apiBaseUrl}/dataset/search?facet=type&facet_only=true&facet_mincount=10000' target='_blank'>/search?facet=type&facet_only=true&facet_mincount=10000</a> only shows the type value 'OCCURRENCE' because 'CHECKLIST' and 'METADATA' have counts less than 10000.",
  "facet_multiselect": "Used in combination with the facet parameter. Set facet_multiselect=true to still return counts for values that are not currently filtered, e.g. <a href='${action.cfg.apiBaseUrl}/dataset/search?facet=type&facet_only=true&type=CHECKLIST&facet_multiselect=true' target='_blank'>/search?facet=type&facet_only=true&type=CHECKLIST&facet_multiselect=true</a> still shows type values 'OCCURRENCE' and 'METADATA' even though type is being filtered by type=CHECKLIST"
  }+params />
<#else>
